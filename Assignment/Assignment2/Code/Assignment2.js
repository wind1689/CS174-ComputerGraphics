...
//Using in navigation system to generate the new "at";
function newAt(v, u, Degree)
{
  var distance = length1(subtract(eye, at));
  var lengthU = distance*Math.sin(radians(Degree));
  var lengthV = distance*(1-Math.cos(radians(Degree)));
  var scaleU = vec3(lengthU,lengthU,lengthU);
  var scaleV = vec3(lengthV,lengthV,lengthV);
  var u2 = mult(scaleU, u);
  var v2 = mult(scaleV, v);
  var m = subtract(u2, v2);          //moving vector for "at";
  return add(at, m);
}
...
window.onkeydown = function(event)
{
    ...
    switch (keyCode)
    {
    	...
    	//Press "Up Arrow"
    	case 38:
          var temp = eyeVertAngle + navDegree;
          if (temp < 90)
          {
            eyeVertAngle = temp;
    	...
    	case 37:
          ...
          var n = normalize( cross(v, up));       // perpendicular vector
          var u = normalize( cross(n, v) );        // "new" up vector
          var newV = normalize( cross(up, n) );     // new view direction vector
          at = newAt(newV, n, -navDegree);
          viewMatrix = lookAt(eye, at, up);
          break;
    	//Press "Right Arrow"
    	case 39:
      ...
      case 66:
    	...
    	case 73:
          var v = normalize(subtract(at, eye));  // view direction vector
          var mVector = mult(vec3(navUnit, navUnit, navUnit), v);
          eye = add(eye, mVector);
          at = add(at, mVector);
          viewMatrix = lookAt(eye, at, up);
    			break;
    	...
    	case 74:
          var v = normalize(subtract(at, eye));  // view direction vector
          var n = normalize( cross(v, up));       // perpendicular vector
          var mVector = mult(vec3(-navUnit, -navUnit, -navUnit), n);
          eye = add(eye, mVector);
          at = add(at, mVector);
          viewMatrix = lookAt(eye, at, up);
    			break;
